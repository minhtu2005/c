...
// khi chạy chương trình thì thầy cô chọn lựa chọn nào thì lướt lên 1 tí tại vì lúc nó hiện kết quả mà mấy cái lựa chọn khác nó từ 1 đến 9 nó xuất hiện sau khi có kết quả nên kết quá sẽ ở phía trên mấy cái lựa chọn
...
void chenSo(int c)//case 8
{
    int i;
    for( i=n-1;(i>=0&&a[i]>c);i--){
        a[i+1]=a[i];
    }
    a[i+1]=c;
    n++;
